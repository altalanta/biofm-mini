...
def test_eval_metrics_ranges(tmp_path: Path) -> None:
    ...
    assert (
        "evaluation" in metrics_data
    ), f"No evaluation section in metrics: {metrics_data.keys()}"
    eval_metrics = metrics_data["evaluation"]

    # Check AUROC metrics
    assert (
        "auroc" in eval_metrics
    ), f"No AUROC in evaluation metrics: {eval_metrics.keys()}"
    ...
    assert (
        auroc_ci_low <= auroc_value <= auroc_ci_high
    ), f"AUROC CI bounds invalid: {auroc_ci_low} <= {auroc_value} <= {auroc_ci_high}"

    # Check AUPRC metrics
    assert (
        "auprc" in eval_metrics
    ), f"No AUPRC in evaluation metrics: {eval_metrics.keys()}"
    ...
    # CI bounds should be valid
    assert (
        auprc_ci_low <= auprc_value <= auprc_ci_high
