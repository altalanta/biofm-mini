...
def bootstrap_predictions(
    ...
    for i in range(n_bootstrap):
        ...
        try:
            ...
            # Compute metrics
            from biofm.eval.metrics import compute_auprc, compute_auroc
...
def main(
    ...
    profile: str = typer.Option(
        ...
        click_type=typer.Choice(["toy", "real"]),
    ),
    ...
    n_bootstrap: int = typer.Option(
        100, "--n-bootstrap", help="Number of bootstrap iterations"
    ),
) -> None:
    ...
    print("\n=== Evaluation Summary ===")
    print(
        f"AUROC: {metrics_output['auroc']['point_estimate']:.3f} "
        f"[{metrics_output['auroc']['ci_low']:.3f}, {metrics_output['auroc']['ci_high']:.3f}]"
    )
    print(
        f"AUPRC: {metrics_output['auprc']['point_estimate']:.3f} "
